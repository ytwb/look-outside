...
var TIMER_DURATION_SECONDS = 1200;
var MOVEMENT_REMINDER_DURATION_SECONDS = 1800;
var EYE_REST_DURATION_SECONDS = 20;
var LOCALE_OPTIONS = [
	{
		value: "en",
		label: "English"
...
var MOVEMENT_REMINDERS = [
	{
		title: {
			en: "Side neck stretch",
			zh: "頸部側壓"
		},
		body: {
	...
	{
		title: {
			en: "Seated Ankle Rolls and Toe Flexes",
			zh: "坐姿腳踝腳趾運動"
		},
		body: {
...
function getSecondsUntil(timestamp) {
	return Math.max(0, Math.ceil((timestamp - Date.now()) / 1e3));
}
...
function meta({}) {
	return [{ title: "Outside | Focus Break Timer" }, {
		name: "description",
...
var home_default = UNSAFE_withComponentProps(function Home() {
	...
	const [latestMovementReminder, setLatestMovementReminder] = useState(null);
	const [eyeRestEndsAt, setEyeRestEndsAt] = useState(null);
	const [movementEndsAt, setMovementEndsAt] = useState(null);
	const [activeEyeRestEndsAt, setActiveEyeRestEndsAt] = useState(null);
	...
	function getNextMovementReminder() {
		setLatestMovementReminder((currentReminder) => {
			...
			while (nextReminder === currentReminder) nextReminder = getMovementReminder();
			return nextReminder;
		});
	}
	useEffect(() => {
		...
		function closeLocaleMenu(event) {
			if (localeMenuRef.current?.contains(event.target)) return;
	...
	useEffect(() => {
		...
		updateTimers();
		const interval = window.setInterval(updateTimers, 1e3);
	...
	useEffect(() => {
		...
		const interval = window.setInterval(updateActiveEyeRestTimer, 1e3);
	...
	useEffect(() => {
		...
		if (isRunning) setEyeRestEndsAt(Date.now() + TIMER_DURATION_SECONDS * 1e3);
		else setEyeRestEndsAt(null);
	}, [
	...
	useEffect(() => {
		...
		if (isMovementDue) {
			setMovementEndsAt(now + MOVEMENT_REMINDER_DURATION_SECONDS * 1e3);
		...
		if (isEyeRestDue && isMovementDue) {
			...
			showReminderNotification(copy.combinedNotificationTitle, copy.combinedNotificationBody.replace("{title}", getLocalizedText(movementReminder.title, locale)).replace("{body}", getLocalizedText(movementReminder.body, locale)), startActiveEyeRest);
			return;
		}
		if (isEyeRestDue) {
		...
		const movementReminder = getMovementReminder();
		setLatestMovementReminder(movementReminder);
		showReminderNotification(getLocalizedText(movementReminder.title, locale), getLocalizedText(movementReminder.body, locale));
	}, [
		copy,
	...
	const notificationStatus = notificationPermission === "granted" ? copy.notificationEnabled : notificationPermission === "denied" ? copy.notificationBlocked : notificationPermission === "unsupported" ? copy.notificationUnsupported : copy.notificationPrompt;
	...
	function pauseActiveEyeRest() {
		if (activeEyeRestEndsAt !== null) setActiveEyeRestSecondsRemaining(getSecondsUntil(activeEyeRestEndsAt));
		setIsEyeRestActive(false);
		isEyeRestActiveRef.current = false;
		setIsEyeRestPending(false);
		isEyeRestPendingRef.current = false;
	...
	function startActiveEyeRest() {
		...
		setActiveEyeRestEndsAt(Date.now() + activeEyeRestSecondsRemaining * 1e3);
		setIsEyeRestPending(true);
		isEyeRestPendingRef.current = true;
		setIsEyeRestActive(true);
		isEyeRestActiveRef.current = true;
	}
	function showReminderNotification(title, body, onClick) {
	...
	async function handleTimerToggle() {
		if (isRunning) {
			if (eyeRestEndsAt !== null) setEyeRestSecondsRemaining(getSecondsUntil(eyeRestEndsAt));
			if (movementEndsAt !== null) setMovementSecondsRemaining(getSecondsUntil(movementEndsAt));
			pauseActiveEyeRest();
		...
		const now = Date.now();
		setEyeRestEndsAt(now + eyeRestSecondsRemaining * 1e3);
		setMovementEndsAt(now + movementSecondsRemaining * 1e3);
		if (activeEyeRestSecondsRemaining > 0 && activeEyeRestSecondsRemaining < EYE_REST_DURATION_SECONDS) {
			setActiveEyeRestEndsAt(now + activeEyeRestSecondsRemaining * 1e3);
			setIsEyeRestActive(true);
			isEyeRestActiveRef.current = true;
			setIsEyeRestPending(true);
			isEyeRestPendingRef.current = true;
	...
	function handleLocaleSelect(selectedLocale) {
		setLocale(selectedLocale);
		setIsLocaleMenuOpen(false);
	}
	return /* @__PURE__ */ jsx("main", {
		className: `timer-page${isDarkMode ? " dark-mode" : ""}`,
			...
			children: [
				/* @__PURE__ */ jsxs("div", {
					...
					children: [/* @__PURE__ */ jsxs("div", {
						...
						ref: localeMenuRef,
						children: [
							/* @__PURE__ */ jsx("span", { children: copy.localeToggle }),
							/* @__PURE__ */ jsx("button", {
							...
							isLocaleMenuOpen && /* @__PURE__ */ jsx("div", {
								className: "language-options",
								role: "listbox",
								"aria-label": copy.localeToggle,
								children: LOCALE_OPTIONS.map((option) => /* @__PURE__ */ jsx("button", {
						...
						children: isDarkMode ? copy.lightMode : copy.darkMode
					})]
				}),
				/* @__PURE__ */ jsx("p", {
					className: "eyebrow",
				...
				/* @__PURE__ */ jsxs("div", {
					...
					"aria-live": "polite",
					"aria-label": copy.eyeRestAria.replace("{minutes}", String(eyeRestMinutes)).replace("{seconds}", String(eyeRestSeconds)),
					children: [/* @__PURE__ */ jsx("span", { children: eyeRestTimerLabel }), /* @__PURE__ */ jsx("small", { children: copy.timerCaption })]
				}),
				/* @__PURE__ */ jsxs("p", {
					className: "movement-timer",
					"aria-live": "polite",
					children: [
						/* @__PURE__ */ jsx("span", { children: copy.moveIn }),
						" ",
						/* @__PURE__ */ jsx("strong", { children: movementTimerLabel })
					]
				}),
				/* @__PURE__ */ jsx("p", {
					className: "notification-status",
					"aria-live": "polite",
					children: notificationStatus
				}),
				isEyeRestPending && !isEyeRestActive && /* @__PURE__ */ jsxs("article", {
					className: "pending-eye-rest-card",
					"aria-live": "assertive",
					children: [/* @__PURE__ */ jsx("p", { children: copy.pendingEyeRest }), /* @__PURE__ */ jsx("button", {
						className: "inline-action-button",
						type: "button",
						onClick: startActiveEyeRest,
						children: copy.startEyeRest
					})]
				}),
				latestMovementReminder && /* @__PURE__ */ jsxs("article", {
					className: "movement-reminder-card",
					"aria-live": "polite",
					children: [
						/* @__PURE__ */ jsx("h2", { children: getLocalizedText(latestMovementReminder.title, locale) }),
						/* @__PURE__ */ jsx("p", { children: getLocalizedText(latestMovementReminder.body, locale) }),
						/* @__PURE__ */ jsx("button", {
							className: "inline-action-button",
							type: "button",
							onClick: getNextMovementReminder,
							children: copy.anotherMovementReminder
						})
					]
				}),
				isEyeRestActive && /* @__PURE__ */ jsxs("p", {
					className: "active-rest-timer",
					"aria-live": "polite",
					children: [
						copy.activeRest,
						" ",
						/* @__PURE__ */ jsx("strong", { children: activeEyeRestTimerLabel })
					]
				}),
				/* @__PURE__ */ jsx("button", {
					...
					children: isRunning ? copy.pauseTimers : eyeRestSecondsRemaining === TIMER_DURATION_SECONDS && movementSecondsRemaining === MOVEMENT_REMINDER_DURATION_SECONDS ? copy.startTimer : copy.resumeTimers
				}),
				/* @__PURE__ */ jsx("button", {
					className: "reset-button",
					type: "button",
					onClick: handleReset,
					disabled: eyeRestSecondsRemaining === TIMER_DURATION_SECONDS && movementSecondsRemaining === MOVEMENT_REMINDER_DURATION_SECONDS && !isRunning,
					children: copy.reset
				}),
				/* @__PURE__ */ jsx("p", {
					className: "status",
					"aria-live": "polite",
					children: isRunning ? copy.bothRemindersRunning : copy.pausedStatus
				})
			]
		})
	});
...
var server_manifest_default = {
	...
	"routes": {
		"root": {
			...
			"module": "/assets/root-DOvUFqZZ.js",
			"imports": ["/assets/jsx-runtime-9DXhWUXR.js", "/assets/errorBoundaries-BAoyOSvd.js"],
			"css": ["/assets/root-87KZrFQG.css"],
		...
		"routes/home": {
			...
			"module": "/assets/home-DlvmiFhI.js",
	...
	"url": "/assets/manifest-77c6b358.js",
	"version": "77c6b358",
	"sri": void 0
};
//#endregion
